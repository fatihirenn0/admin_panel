<?php
$locales = \App\Models\Locale::orderBy('rank','asc')->get();
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <script>
        jQuery(document).ready(function($){
            ...
            $('.group-select').on('change', function(){
                window.location.href = '<?php echo action('\Barryvdh\TranslationManager\Controller@getView') ?>/_json';
                return false;
                var group = $(this).val();
                if (group) {
...
<!--                <span class="sr-only">Gezinmeyi aç/kapa</span>-->
...
<!--</header>-->
<div class="container-fluid">
    <!--    <p>Uyarı, çeviriler, <code>php artisan translation:export</code> komutu veya yayınla butonu kullanılarak app/lang dosyasına geri aktarılana kadar görünür olmayacaktır.</p>-->
    <div class="alert alert-success success-import" style="display:none;">
        <p>İçe aktarma işlemi tamamlandı, <strong class="counter">N</strong> öğe işlendi! Grupları yenilemek için bu sayfayı yeniden yükleyin!</p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p>Çeviriler için arama tamamlandı, <strong class="counter">N</strong> öğe bulundu!</p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p>'<?php echo $group ?>' grubu için çeviriler yayınlandı!</p>
    </div>
    <div class="alert alert-success success-publish-all" style="display:none;">
        <p>Tüm gruplar için çeviriler yayınlandı!</p>
    </div>
    ...
    <form class="form-import" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
        ...
        <div class="form-group">
            <div class="row">
                ...
                <div class="col-sm-2">
                    <button type="submit" class="btn btn-success btn-block"  data-disable-with="Yükleniyor..">Grupları içe aktar</button>
                </div>
            </div>
        </div>
    </form>
    <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Uygulama klasörünüzü taramak istediğinizden emin misiniz? Bulunan tüm çeviri anahtarları veritabanına eklenecektir.">
        <div class="form-group">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-info" data-disable-with="Aranıyor.." >Dosyalarda çevirileri bul</button>
        </div>
    </form>
<?php endif; ?>
    <?php if(isset($group)) : ?>
        <form class="form-inline form-publish" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', $group) ?>" data-remote="true" role="form" data-confirm="'<?php echo $group ?>' grubunun çevirilerini yayınlamak istediğinizden emin misiniz? Bu, mevcut dil dosyalarının üzerine yazacaktır.">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <button type="submit" class="btn btn-primary" data-disable-with="Yayınlanıyor.." >Çevirileri yayınla</button>
            <!--            <a href="--><?php //= action('\Barryvdh\TranslationManager\Controller@getIndex') ?><!--" class="btn btn-default">Geri</a>-->
        </form>
    <?php endif; ?>
    </p>
    <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <!--        <div class="form-group">-->
        <!--            <p>Grup çevirilerini görüntülemek için bir grup seçin. Hiçbir grup görünmüyorsa, göçlerin çalıştırıldığından ve çevirilerin içe aktarıldığından emin olun.</p>-->
        <!--            <select name="group" id="group" onload="this.trigger('change')" class="form-control group-select">-->
        <!--                --><?php //foreach($groups as $key => $value): ?>
        <!--                    <option value="--><?php //echo $key ?><!--"--><?php //echo $key == '_json' ? ' selected':'' ?><!--<?php //echo $value ?><!--</option>-->
        <!--                --><?php //endforeach; ?>
        <!--            </select>-->
        <!--        </div>-->
        <!--        <div class="form-group">-->
        ...
        <!--        </div>-->
        <!--        <div class="form-group">-->
        <!--            <input type="submit" class="btn btn-default" name="add-group" value="Anahtarları ekle ve düzenle" />-->
        <!--        </div>-->
    </form>
    <?php if($group): ?>
        ...
        <!--            <input type="hidden" name="_token" value="--><?php //echo csrf_token(); ?><!--">-->
        <!--            <div class="form-group">-->
        <!--                <label>Bu gruba yeni anahtarlar ekle</label>-->
        <!--                <textarea class="form-control" rows="3" name="keys" placeholder="Grup öneki olmadan anahtar başına 1 anahtar ekleyin"></textarea>-->
        <!--            </div>-->
        <!--            <div class="form-group">-->
        <!--                <input type="submit" value="Anahtarları ekle" class="btn btn-primary">-->
        <!--            </div>-->
        <!--        </form>-->
        <!--        <div class="row">-->
        <!--            <div class="col-sm-2">-->
        ...
        <!--                                <option value="--><?php //= $locale ?><!--">--><?php //= $locale ?><!--</option>-->
        <!--                            --><?php //endforeach; ?>
        <!--                        </select>-->
        <!--                    </div>-->
        <!--                    <div class="form-group">-->
        <!--                        <label for="new-locale">Hedef yerel anahtarını girin</label>-->
        <!--                        <input type="text" name="new-locale" class="form-control" id="new-locale" placeholder="Hedef yerel anahtarını girin" />-->
        <!--                    </div>-->
        <!--                    --><?php //if(!config('laravel_google_translate.google_translate_api_key')): ?>
        <!--                        <p>-->
        <!--                            <code>Google Çeviri API'sini kullanmak istiyorsanız, tanmuhittin/laravel-google-translate'i yükleyin ve Google Çeviri API anahtarınızı laravel_google_translate yapılandırma dosyasına girin</code>-->
        <!--                        </p>-->
        <!--                    --><?php //endif; ?>
        <!--                    <div class="form-group">-->
        <!--                        <input type="hidden" name="with-translations" value="1">-->
        <!--                        <input type="hidden" name="file" value="--><?php //= $group ?><!--">-->
        <!--                        <button type="submit" class="btn btn-default btn-block"  data-disable-with="Ekleniyor..">Eksik çevirileri otomatik çevir</button>-->
        <!--                    </div>-->
        <!--                </div>-->
        <!--            </div>-->
        <!--        </form>-->
        <hr>
        <h4>Toplam: <?= $numTranslations ?>, değiştirildi: <?= $numChanged ?></h4>
        <input type="text" id="searchInput" placeholder="Anahtar veya çeviri ara..." class="form-control" style="margin-bottom: 10px; width: 100%;">
        <table class="table">
            <thead style="position: sticky; top: 0; background: #fff;">
            <tr>
                <th width="15%">Anahtar</th>
                <?php foreach ($locales as $locale): ?>
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                    ...
                    <?php foreach ($locales as $locale): ?>
                        <?php $t = isset($translation[$locale['locale']]) ? $translation[$locale['locale']] : null ?>

                        <td>
                            <a href="#edit"
                               class="editable status-<?php echo $t ? $t->status : 0 ?> locale-<?php echo $locale['locale'] ?>"
                               data-locale="<?php echo $locale['locale'] ?>" data-name="<?php echo $locale['locale'] . "|" . htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>"
                               id="username" data-type="textarea" data-pk="<?php echo $t ? $t->id : 0 ?>"
                               data-url="<?php echo $editUrl ?>"
                               data-title="Çeviriyi girin"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                        </td>
                    <?php endforeach; ?>
                    <?php if (!$deleteEnabled): ?>
                        <td>
                            <a href="<?php echo action('\Barryvdh\TranslationManager\Controller@postDelete', [$group, $key]) ?>"
                               class="delete-key"
                               data-confirm="'<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>' için çevirileri silmek istediğinizden emin misiniz?"><span
    ...
    <?php else: ?>
        ...
        <!--            </p>-->
        <!--            <form  class="form-remove-locale" method="POST" role="form" action="--><?php //echo action('\Barryvdh\TranslationManager\Controller@postRemoveLocale') ?><!--" data-confirm="Bu yerel ayarı ve tüm verilerini kaldırmak istediğinizden emin misiniz?">-->
        <!--                <input type="hidden" name="_token" value="--><?php //echo csrf_token(); ?><!--">-->
        ...
        <!--                        </div>-->
        <!--                        <div class="col-sm-2">-->
        <!--                            <button type="submit" class="btn btn-default btn-block"  data-disable-with="Ekleniyor..">Yeni yerel ayar ekle</button>-->
        <!--                        </div>-->
        <!--                    </div>-->
        <!--                </div>-->
        <!--            </form>-->
        ...
        <fieldset>
            ...
            <form class="form-inline form-publish-all" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish', '*') ?>" data-remote="true" role="form" data-confirm="Tüm çeviri grubunu yayınlamak istediğinizden emin misiniz? Bu, mevcut dil dosyalarının üzerine yazacaktır.">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                <button type="submit" class="btn btn-primary" data-disable-with="Yayınlanıyor.." >Tümünü yayınla</button>
            </form>
        </fieldset>
...
<script>
    document.getElementById('searchInput').addEventListener('keyup', function () {
        ...
        rows.forEach(function (row) {
            ...
            var translationCells = row.querySelectorAll('td a');

            var match = keyCell && keyCell.textContent.toLowerCase().includes(input);
