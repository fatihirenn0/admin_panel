...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <form id="mainForm" method="post" action="{{ route('admin.references.update',$reference) }}" enctype="multipart/form-data">
            @csrf @method('put')
            <div class="row g-6">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Referans Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $reference->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                            ...
                                            <div class="col-md-12 mt-2">
                                                @include('inputs.file',[
                                                    ...
                                                    'value' => '/storage/'.$reference->getTranslation('image',$locale->locale)
                                                ])
                                            </div>
                                        @endif
                                    </div>
