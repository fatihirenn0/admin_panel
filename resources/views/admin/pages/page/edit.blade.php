...
@section('parent_menu', __('Sayfalar'))
@section('parent_menu_link', route('admin.pages.index'))
@section('title', __('Sayfa Düzenle'))
@push('css')

@endpush
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <form id="mainForm" method="post" action="{{ route('admin.pages.update',$page) }}" enctype="multipart/form-data">
            @csrf @method('put')
            <div class="row g-6">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Sayfa Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $page->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.editor',[
                                                'title'=>__('Sayfa Açıklaması') . " ({$locale->language})",
                                                'name'=>"description[{$locale->locale}]",
                                                'loopIndex' => $loop->index,
                                                'id' => 'name_'.$locale->locale,
                                                'value' => $page->getTranslation('description',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                            <div class="col-md-12 mt-2">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                ...
                                                'cropHeight' => 800,
                                                'loopIndex' => $loop->index,
                                                'value' => '/storage/'.$page->getTranslation('image',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                @for($i = 0;$i < 10;$i++)
                                                    @php $pageImage = $pageImages->where('locale_id',$locale->id)->skip($i)->take(1)->first() @endphp
                                                    <tr class="{{ $pageImage ? '' : 'd-none' }} images-{{ $locale->locale }}">
                                                        <td>
                                                            @include('inputs.file',[
                                                               ...
                                                               'loopIndex' => $i + 1,
                                                               'dataHeight' => 100,
                                                               'value' => '/storage/'.$pageImage?->image_url
                                                           ])
                                                            @if($pageImage)
                                                                <input type="hidden" name="old_image_ids[{{ $locale->id }}][{{ $i }}]" value="{{ $pageImage->id }}">
                                                            @endif
                                                        </td>
                                                        <td>
                                                            <input type="number" class="form-control-sm form-control" min="0" name="image_ranks[{{ $locale->id }}][{{ $i }}]" value="{{ $pageImage ? $pageImage->rank : $i+1 }}">
                                                        </td>
                                                    </tr>
                                                @endfor
                                                </tbody>
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $page->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $page->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                    </div>
                                </div>
