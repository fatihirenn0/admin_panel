        ...
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(isset($_GET['trashed']))
                    <a href="{{ route('admin.projects.index') }}" class="btn btn-info"><i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Projeler') }}</a>
                @else
                    <a href="{{ route('admin.projects.index', ['trashed'=>true]) }}" class="btn btn-danger"><i class="menu-icon icon-base ti tabler-recycle"></i>{{ __('Geri Dönüşüm') }} </a>
                @endif
