...
@section('parent_menu', __('Projeler'))
@section('parent_menu_link', route('admin.projects.index'))
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <form id="mainForm" method="post" action="{{ route('admin.projects.update',$project) }}" enctype="multipart/form-data">
            @csrf @method('put')
            <div class="row g-6">
                                        ...
                                        @if($loop->first)
                                            <div class="col-md-12">
                                                @include('inputs.select',[
                                                    'options' => $projectCategories->pluck('name','id')->toArray(),
                                                    'title' => __('Kategoriler'),
                                                    'multiple' => true,
                                                    'name' => 'project_categories[]',
                                                    'loopIndex' => $loop->index,
                                                    'selected' => $projectCategoryIds
                                                ])
                                            </div>
                                        @endif
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Proje Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $project->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.editor',[
                                                'title'=>__('Proje Açıklaması') . " ({$locale->language})",
                                                'name'=>"description[{$locale->locale}]",
                                                'loopIndex' => $loop->index,
                                                'id' => 'name_'.$locale->locale,
                                                'value' => $project->getTranslation('description',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                            <div class="col-md-12 mt-2">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                ...
                                                'cropHeight' => 800,
                                                'loopIndex' => $loop->index,
                                                'value' => '/storage/'.$project->getTranslation('image',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                @for($i = 0;$i < 10;$i++)
                                                    @php $projectImage = $projectImages->where('locale_id',$locale->id)->skip($i)->take(1)->first() @endphp
                                                    <tr class="{{ $projectImage ? '' : 'd-none' }} images-{{ $locale->locale }}">
                                                        <td>
                                                            @include('inputs.file',[
                                                               ...
                                                               'loopIndex' => $i + 1,
                                                               'dataHeight' => 100,
                                                               'value' => '/storage/'.$projectImage?->image_url
                                                           ])
                                                            @if($projectImage)
                                                                <input type="hidden" name="old_image_ids[{{ $locale->id }}][{{ $i }}]" value="{{ $projectImage->id }}">
                                                            @endif
                                                        </td>
                                                        <td>
                                                            <input type="number" class="form-control-sm form-control" min="0" name="image_ranks[{{ $locale->id }}][{{ $i }}]" value="{{ $projectImage ? $projectImage->rank : $i+1 }}">
                                                        </td>
                                                    </tr>
                                                @endfor
                                                </tbody>
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $project->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $project->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                    </div>
                                </div>
