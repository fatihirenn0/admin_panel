    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('announcement_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.announcements.index') }}" class="btn btn-info"><i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Duyurular') }}</a>
                    @else
                        <a href="{{ route('admin.announcements.index', ['trashed'=>true]) }}" class="btn btn-danger"><i class="menu-icon icon-base ti tabler-recycle"></i>{{ __('Geri Dönüşüm') }} </a>
                    @endif
                @endif
                @if(in_array('announcement_add', $authUserRoles))
                    <a href="{{ route('admin.announcements.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
...
@push('js')
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.ajax.announcements') }}',
            ...
            columns: [
                ...
                { data: 'text', orderable: true, searchable: true },
