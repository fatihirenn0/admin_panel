...
@section('title',__('Ürün Kategorileri'))
@push('css')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('product_category_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.product-categories.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Ürün Kategorileri') }}
                        </a>
                    @else
                        <a href="{{ route('admin.product-categories.index', ['trashed'=>true]) }}" class="btn btn-danger">
                            <i class="menu-icon icon-base ti tabler-recycle"></i>{{ __('Geri Dönüşüm') }}
                        </a>
                    @endif
                @endif
                @if(in_array('product_category_add', $authUserRoles))
                    <a href="{{ route('admin.product-categories.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
...
@push('js')
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.ajax.product-categories') }}',
