...
@section('title',__('Dosya Yönetimi'))
@push('css')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('file_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.files.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Dosya Yönetimi') }}
                        </a>
                    @else
                        <a href="{{ route('admin.files.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('file_add', $authUserRoles))
                    <a href="{{ route('admin.files.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            columns: [
                ...
                { data: 'file_url', orderable: true, searchable: true },
