    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('faq_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.faqs.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Soru Listesi') }}
                        </a>
                    @else
                        <a href="{{ route('admin.faqs.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('faq_add', $authUserRoles))
                    <a href="{{ route('admin.faqs.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            columns: [
                ...
                { data: 'answer', orderable: true, searchable: true },
