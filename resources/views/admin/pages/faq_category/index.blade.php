    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('faq_category_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.faq-categories.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Soru Kategorileri') }}
                        </a>
                    @else
                        <a href="{{ route('admin.faq-categories.index', ['trashed'=>true]) }}" class="btn btn-danger">
