...
@section('title',__('Fotoğraf Kategorileri'))
@push('css')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('photo_category_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.photo-categories.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Fotoğraf Kategorileri') }}
                        </a>
                    @else
                        <a href="{{ route('admin.photo-categories.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('photo_category_add', $authUserRoles))
                    <a href="{{ route('admin.photo-categories.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
            ...
            <div class="card-datatable text-nowrap">
                <table class="datatables-ajax table table-bordered">
                    ...
                    <tr>
                        <th>{{ __('ID') }}</th>
                        <th>{{ __('Görsel') }}</th>
                        <th>{{ __('Fotoğraf Kategori Adı') }}</th>
...
@push('js')
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.ajax.photo-categories') }}',
