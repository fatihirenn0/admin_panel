...
@section('parent_menu', __('Tarihçe'))
@section('parent_menu_link', route('admin.milestones.index'))
@section('title',__('Tarihçe Düzenle'))
@push('css')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row g-6">
            <form id="mainForm" method="post" action="{{ route('admin.milestones.update',$milestone) }}" enctype="multipart/form-data">
                @csrf @method('put')
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Tarihçe Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $milestone->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                'title'=>__('Tarihçe Açıklaması') . " ({$locale->language})",
                                                'name'=>"description[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $milestone->getTranslation('description',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                            <div class="col-md-12 mt-2">
                                            ...
                                            <div class="col-md-12 mt-2">
                                                @include('inputs.input',[
                                                    ...
                                                    'type'=>'number',
                                                    'name'=>'rank',
                                                    'value' => $milestone->rank
                                                ])
                                            </div>
                                        @endif
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                'title'=>__('Resim') . " ({$locale->language})",
                                                'name'=>"image[{$locale->locale}]",
                                                'cropWidth' => 1200,
                                                'cropHeight' => 800,
                                                'value' => '/storage/'.$milestone->getTranslation('image',$locale->locale),
                                                'loopIndex' => 0
                                            ])
                                        </div>
                                    </div>
