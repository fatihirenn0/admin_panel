...
@section('title',__('Tarihçe Listesi'))
@push('css')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('milestone_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.milestones.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Tarihçe Listesi') }}
                        </a>
                    @else
                        <a href="{{ route('admin.milestones.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('milestone_add', $authUserRoles))
                    <a href="{{ route('admin.milestones.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
            ...
            <div class="card-datatable text-nowrap">
                <table class="datatables-ajax table table-bordered">
                    ...
                    <tr>
                        <th>{{ __('ID') }}</th>
                        <th>{{ __('Tarihçe Adı') }}</th>
                        <th>{{ __('Tarihçe Açıklaması') }}</th>
                        <th>{{ __('Görsel') }}</th>
                        <th>{{ __('Tarih') }}</th>
...
@push('js')
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('admin.ajax.milestones') }}',
