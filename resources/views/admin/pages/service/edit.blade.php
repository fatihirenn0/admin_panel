...
@section('parent_menu', __('Hizmetler'))
@section('parent_menu_link', route('admin.services.index'))
@section('title',__('Hizmet Düznle'))
@push('css')

@endpush
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <form id="mainForm" method="post" action="{{ route('admin.services.update',$service) }}" enctype="multipart/form-data">
            @csrf @method('put')
            <div class="row g-6">
                                        ...
                                        @if($loop->first)
                                            <div class="col-md-12">
                                                @include('inputs.select',[
                                                    ...
                                                    'selected' => $serviceCategoryIds
                                                ])
                                            </div>
                                        @endif
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Hizmet Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $service->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.editor',[
                                                'title'=>__('Hizmet Kısa Açıklaması') . " ({$locale->language})",
                                                'name'=>"short_description[{$locale->locale}]",
                                                'loopIndex' => $loop->index,
                                                'id' => 'short_description_'.$locale->locale,
                                                'value' => $service->getTranslation('short_description',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.editor',[
                                                'title'=>__('Hizmet Tam Açıklaması') . " ({$locale->language})",
                                                'name'=>"long_description[{$locale->locale}]",
                                                'loopIndex' => $loop->index,
                                                'id' => 'long_description_'.$locale->locale,
                                                'value' => $service->getTranslation('long_description',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                            <div class="col-md-12 mt-2">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                ...
                                                'cropHeight' => 800,
                                                'loopIndex' => $loop->index,
                                                'value' => '/storage/'.$service->getTranslation('cover',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                'title'=>__('Alt Resim') . " ({$locale->language})",
                                                'name'=>"image[{$locale->locale}]",
                                                'cropWidth' => 1200,
                                                'cropHeight' => 800,
                                                'loopIndex' => $loop->index,
                                                'value' => '/storage/'.$service->getTranslation('image',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                @for($i = 0;$i < 10;$i++)
                                                    @php $serviceImage = $serviceImages->where('locale_id',$locale->id)->skip($i)->take(1)->first() @endphp
                                                    <tr class="{{ $serviceImage ? '' : 'd-none' }} images-{{ $locale->locale }}">
                                                        <td>
                                                            @include('inputs.file',[
                                                               ...
                                                               'loopIndex' => $i + 1,
                                                               'dataHeight' => 100,
                                                               'value' => '/storage/'.$serviceImage?->image_url
                                                           ])
                                                            @if($serviceImage)
                                                                <input type="hidden" name="old_image_ids[{{ $locale->id }}][{{ $i }}]" value="{{ $serviceImage->id }}">
                                                            @endif
                                                        </td>
                                                        <td>
                                                            <input type="number" class="form-control-sm form-control" min="0" name="image_ranks[{{ $locale->id }}][{{ $i }}]" value="{{ $serviceImage ? $serviceImage->rank : $i+1 }}">
                                                        </td>
                                                    </tr>
                                                @endfor
                                                </tbody>
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $service->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $service->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                    </div>
                                </div>
