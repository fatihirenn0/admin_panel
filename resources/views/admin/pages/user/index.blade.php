...
@section('title',__('Kullanıcı Listesi'))
@push('css')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('user_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.users.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Kullanıcı Listesi') }}
                        </a>
                    @else
                        <a href="{{ route('admin.users.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('user_add', $authUserRoles))
                    <a href="{{ route('admin.users.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
            ...
            <div class="card-datatable text-nowrap">
                <table class="datatables-ajax table table-bordered">
                    ...
                    <tr>
                        <th>{{ __('ID') }}</th>
                        <th>{{ __('Ad Soyad') }}</th>
                        <th>{{ __('Telefon Numarası') }}</th>
                        <th>{{ __('E-posta Adresi') }}</th>
                        <th>{{ __('Yetki Grubu') }}</th>
                        <th>{{ __('İşlemler') }}</th>
                    </tr>
                    </thead>
                </table>
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            columns: [
                ...
                { data: 'role_group', orderable: true, searchable: true },
