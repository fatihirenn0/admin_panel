    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('product_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.products.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Ürünler') }}
                        </a>
                    @else
                        <a href="{{ route('admin.products.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('product_add', $authUserRoles))
                    <a href="{{ route('admin.products.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
