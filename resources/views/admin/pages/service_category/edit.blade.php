...
@section('parent_menu', __('Hizmet Kategorileri'))
@section('parent_menu_link', route('admin.service-categories.index'))
@section('title',__('Hizmet Kategori Düzenle'))
@push('css')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row g-6">
            <form id="mainForm" method="post" action="{{ route('admin.service-categories.update',$serviceCategory) }}" enctype="multipart/form-data">
                @csrf @method('put')
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Hizmet Kategori Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $serviceCategory->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $serviceCategory->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $serviceCategory->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                ...
                                                'cropWidth' => 1200,
                                                'cropHeight' => 800,
                                                'value' => '/storage/'.$serviceCategory->getTranslation('image',$locale->locale),
                                                'loopIndex' => 0
                                            ])
                                        </div>
                                    </div>
