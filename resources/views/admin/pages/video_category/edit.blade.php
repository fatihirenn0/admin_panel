...
@section('parent_menu', __('Video Kategorileri'))
@section('parent_menu_link', route('admin.video-categories.index'))
@section('title',__('Video Kategori Düzenle'))
@push('css')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row g-6">
            <form id="mainForm" method="post" action="{{ route('admin.video-categories.update',$videoCategory) }}" enctype="multipart/form-data">
                @csrf @method('put')
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Video Kategori Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $videoCategory->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $videoCategory->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $videoCategory->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                    </div>
                                </div>
