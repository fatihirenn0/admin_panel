...
@section('parent_menu', __('Proje Kategorileri'))
@section('parent_menu_link', route('admin.project-categories.index'))
@section('title',__('Proje Kategori Düzenle'))
@push('css')
...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row g-6">
            <form id="mainForm" method="post" action="{{ route('admin.project-categories.update',$projectCategory) }}" enctype="multipart/form-data">
                @csrf @method('put')
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            @include('inputs.input',[
                                                'title'=>__('Proje Kategori Adı') . " ({$locale->language})",
                                                'name'=>"name[{$locale->locale}]",
                                                'required' => (bool)$loop->first,
                                                'value' => $projectCategory->getTranslation('name',$locale->locale)
                                            ])
                                        </div>
                                        @if($loop->first)
                                        ...
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $projectCategory->getTranslation('meta_keywords',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.textarea',[
                                                ...
                                                'value' => $projectCategory->getTranslation('meta_description',$locale->locale)
                                            ])
                                        </div>
                                        <div class="col-md-12 mt-2">
                                            @include('inputs.file',[
                                                ...
                                                'cropWidth' => 1200,
                                                'cropHeight' => 800,
                                                'value' => '/storage/'.$projectCategory->getTranslation('image',$locale->locale),
                                                'loopIndex' => 0
                                            ])
                                        </div>
                                    </div>
