    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="card">
            <h5 class="card-header pb-0 d-flex justify-content-between">
                @if(in_array('catalog_delete', $authUserRoles))
                    @if(isset($_GET['trashed']))
                        <a href="{{ route('admin.catalogs.index') }}" class="btn btn-info">
                            <i class="menu-icon icon-base ti tabler-arrow-left"></i>{{ __('Kataloglar') }}
                        </a>
                    @else
                        <a href="{{ route('admin.catalogs.index', ['trashed'=>true]) }}" class="btn btn-danger">
                ...
                @if(in_array('catalog_add', $authUserRoles))
                    <a href="{{ route('admin.catalogs.create') }}" class="btn btn-primary">{{ __('Yeni Kayıt Ekle') }}</a>
                @endif
            </h5>
            ...
            <div class="card-datatable text-nowrap">
                <table class="datatables-ajax table table-bordered">
                    ...
                    <tr>
                        <th>{{ __('ID') }}</th>
                        <th>{{ __('Kategori') }}</th>
                        <th>{{ __('Katalog Adı') }}</th>
                        <th>{{ __('Kapak Resmi') }}</th>
    ...
    <script>
        let dt_ajax = new DataTable(document.querySelector('.datatables-ajax'), {
            ...
            columns: [
                ...
                { data: 'catalog_category_id', orderable: false, searchable: true },
