...
@section('content')
    <section class="page-header">
        ...
        <!-- /.page-header__shape -->
        <div class="container">
            <h2 class="page-header__title bw-split-in-right">{{ $project->name }}</h2>
    ...
    <section class="project-details">
        <div class="container">
            <div class="project-details__topbar">
                <h3 class="project-details__title">{{ $project->name }}</h3>
                <a href="#" class="procounsel-btn"><i>{{ \Carbon\Carbon::parse($project->created_at)->translatedFormat('d F') }}</i><span>{{ \Carbon\Carbon::parse($project->created_at)->translatedFormat('d F') }}</span></a>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-4">
                    <div class="project-details__right">
                        <ul class="project-details__right__list">
                            <li>
                                <p class="project-details__right__lable">{{ __('Müşteri') }}:</p>
                                <span class="project-details__right__text">{{ $project->client }}</span>
                            </li>
                            <li>
                                <p class="project-details__right__lable">{{ __('Şehir') }}:</p>
                                <span class="project-details__right__text">{{ $project->city }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            @php $previousProject = \App\Models\Project::where('id','<',$project->id)->first(); $nextProject = \App\Models\Project::where('id','>',$project->id)->first(); @endphp @if($previousProject)
                <div class="project-details__control">
                    <div class="project-details__control__prev">
                        <a href="{{ route(getResourceFullLink('projects','show'),$previousProject) }}">
                            <div class="project-details__control__icon">
                    ...
                    @endif @if($nextProject)
                        <div class="project-details__control__next">
                            <a href="{{ route(getResourceFullLink('projects','show'),$nextProject) }}">
                                <span>{{ __('Sonraki') }}</span>
                                <div class="project-details__control__icon">
    ...
    <!-- Portfolio Start -->
    <section class="portfolio-two" id="caseStudy">
        <div class="container">
            <div class="sec-title text-center">
                ...
                <h3 class="sec-title__title bw-split-in-up">
                    {{ __('Başarıyla Çözüme Kavuşturduğumuz') }}<br />
                    <span> {{ __('Hukuki Davalar ve Süreçler') }}</span>
                </h3>
                <!-- /.sec-title__title -->
            </div>
            <!-- /.sec-title -->
        </div>
        <div class="container portfolio-two__container wow fadeInUp" data-wow-delay="100ms">
            <div
                class="portfolio-two__carousel procounsel-owl__carousel procounsel-owl__carousel--basic-nav owl-carousel"
                data-owl-options='{
				"items": 1,
				"margin": 30,
				"loop": true,
                ...
                @foreach(\App\Models\Project::where('id' , '!=' , $project->id)->inRandomOrder()->take(6)->get() as $otherProject)
                    <div class="item">
                        <div class="portfolio-two__item">
                            <img src="/storage/{{ $otherProject->image }}" alt="{{ $otherProject->name }}" />
                            <div class="portfolio-two__item__bg"></div>
                            <div class="portfolio-two__item__content">
                                @if($project->categories) @foreach($project->categories as $categoryRelation)
                                    <div class="portfolio-two__item__cat"><i class="icon-pin"></i>{{ $categoryRelation->name }}</div>
                                    <h3 class="portfolio-two__item__title">
                                        <a href="{{ route(getResourceFullLink('projects','show'),$otherProject) }}"> {{ $otherProject->name }}</a>
                                    </h3>
                                    <div class="portfolio-two__item__rm">
                                        <a href="{{ route(getResourceFullLink('projects','show'),$otherProject) }}"><i class="icon-right-arrow-1-4"></i></a>
                                    </div>
                            </div>
                            @endforeach @endif
                        </div>
