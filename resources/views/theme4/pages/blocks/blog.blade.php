    ...
    <div class="container">
        <div class="sec-title text-center">
            ...
            <h3 class="sec-title__title bw-split-in-up">
                {{ __('Hukuki süreçleri anlaşılır ve güvenilir') }} <br />
                <span>{{ __(' Bir şekilde aktarıyoruz.') }}</span>
            </h3>
            <!-- /.sec-title__title -->
        </div>
        <!-- /.sec-title -->
        <div
            class="blog-one__carousel procounsel-owl__carousel procounsel-owl__carousel--with-shadow procounsel-owl__carousel--basic-nav owl-carousel owl-theme"
            data-owl-options='{
				"items": 1,
				"margin": 30,
				"loop": false,
                    ...
                    <div class="blog-card wow fadeInUp" data-wow-duration="1500ms" data-wow-delay="00ms">
                            ...
                            <div class="blog-card__user">
                                <div class="blog-card__user__info">
                                    @if($indexBlog->categories) @foreach($indexBlog->categories as $categoryRelation)
                                        <p class="blog-card__user__designation">{{ $categoryRelation->name }}</p>
                                    @endforeach @endif
                                </div>
                            </div>
                            <h3 class="blog-card__title"><a href="{{ route(getResourceFullLink('blogs','show'),$indexBlog) }}">{{ $indexBlog->name }} </a></h3>
                            <!-- /.blog-card__title -->
                        </div>
                        <!-- /.blog-card__content -->
                        <div class="blog-card__image">
                            <div class="blog-card__image__inner">
                                ...
                                <img src="/storage/{{ $indexBlog->image }}" alt="{{ $indexBlog->name }} " />
                                <a href="{{ route(getResourceFullLink('blogs','show'),$indexBlog) }}" class="blog-card__image__link"><span class="sr-only">{{ $indexBlog->name }} </span></a>
                            </div>
                            <div class="blog-card__date"><span>{{ $indexBlog->created_at->translatedFormat('d') }}</span>{{ $indexBlog->created_at->translatedFormat('F') }}</div>
                            <!-- /.blog-card__date -->
                        </div>
                        <!-- /.blog-card__image -->
                        <ul class="list-unstyled blog-card__meta">
                            <li><a href="{{ route(getResourceFullLink('blogs','show'),$indexBlog) }}">{{ __('İncele') }}</a></li>
                        </ul>
